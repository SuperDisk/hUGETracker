...
function SingleLineCommentText(const Value: String): String;
begin
...
procedure AddSongMetadataHeader(Lines: TStrings; const Song: TSong;
...
begin
  ...
  Lines.Add(CommentPrefix + ' Song title: ' +
    SingleLineCommentText(Song.Name));
  Lines.Add(CommentPrefix + ' Author: ' +
    SingleLineCommentText(Song.Artist));
  Lines.Add(CommentPrefix + ' Exported from hUGETracker ' + Version);
  Lines.Add('');
end;
...
begin
  ...
  for I := Low(OrderMatrix) to High(OrderMatrix) do begin
    ...
    for J := Low(OrderMatrix[I]) to High(OrderMatrix[I])-1 do begin
      ...
      for PatternRow := Low(TPattern) to Song.PatternLength - 1 do begin
        Cell := Pat^[PatternRow];
...
begin
  ...
  try
    ...
    for PatternIndex := 0 to Song.Patterns.Count - 1 do begin
      ...
      for CellIndex := Low(TPattern) to Song.PatternLength - 1 do begin
...
function EncodePattern(const Pattern: TPattern; Channel: TChannel;
  ...
  const UsedStuff: TUsedStuff; PatternLength: Integer): TEncodedPattern;
var
...
begin
  ...
  for Row := Low(TPattern) to PatternLength - 1 do begin
...
var
  ...
  begin
    if Row >= Song.PatternLength then
...
begin
  ...
  AddSongMetadataHeader(OutSL, Song, '//');
  ...
  OutSL.Add(Format(
    'const hUGESong_t %s = {%d, %d, %d, %d, %d, %d, order1, order2, order3,'+
    ...
    [DescriptorName,
     ...
     OrderCount(Song)*2, Song.PatternLength
    ]));
...
begin
  ...
  OutSL.Add('db '+IntToStr(Song.PatternLength));
  ...
  try
    ...
    if Assemble(Filename + '_song.obj',
                ...
                ['SONG_DESCRIPTOR=song',
                 ...
                 'PATTERN_LENGTH='+IntToStr(Song.PatternLength),
